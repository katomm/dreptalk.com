...
// extension exposes (icon is a data URI), so this is a custom radiogroup of
// rows: logo, name, and a CIP-95 badge, with the selected row marked. It only
// reports the chosen key; connecting stays with each flow's own button.
...
export default function WalletPicker({ wallets, selected, onSelect, disabled, label = 'Wallet' }: WalletPickerProps) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
      ...
      <div role="radiogroup" aria-label={label} style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
        {wallets.map((w) => {
          ...
          return (
            ...
            >
              ...
              {/* Selected check; reserves no layout when absent (badge stays put). */}
              {isSelected && (
