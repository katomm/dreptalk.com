...
Re-delegating restarts the clock, including a re-delegation to the same DRep. That is why the page says "since your latest delegation in epoch N" rather than "since you first delegated". The epoch is read once from the on-chain history of your account and refreshed daily for accounts that do not have it yet. Until it is on record, the page shows nothing that would count from it and says so. If several lookups in a row come back empty, with no delegation certificate in the account history, the page stops waiting and says the start is not on record, pointing you to the all-time record on your DRep's profile or, for a default option, listing the most recent decided actions without a window. A lookup that fails to run at all does not count toward this, only one that completes and comes back empty does.
