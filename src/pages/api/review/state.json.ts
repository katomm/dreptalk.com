...
export const GET: APIRoute = async ({ request, locals }) => {
  ...
  return withEdgeCache(
    ...
    async () => {
      ...
      const body = await buildReviewState(db, cfg, {
        lastCoveredEpoch: index.lastCoveredEpoch,
        nowMs: Date.now(),
      });
      return jsonResponse(body, 200, { 'Cache-Control': `public, max-age=60, s-maxage=${TTL}` });
    },
    '/api/review/state.json',
  );
};
