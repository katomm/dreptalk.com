...
export function factCheckEdition(input: { frontmatter: ReviewFrontmatter; body: string; pack: unknown }): Finding[] {
  ...
  for (const m of body.matchAll(CHART_RE)) {
    ...
    if (spec.type === 'budget') { for (const [i, g] of spec.segments.entries()) segments.push({ label: `chart ${n} segment ${i + 1} label`, text: g.label }); segments.push({ label: `chart ${n} total label`, text: spec.total.label }); if (spec.marker) segments.push({ label: `chart ${n} marker label`, text: spec.marker.label }); }
    if (spec.type === 'compare') for (const [k, p] of spec.panels.entries()) { segments.push({ label: `chart ${n} panel ${k + 1} title`, text: p.title }); for (const [i, r] of p.rows.entries()) segments.push({ label: `chart ${n} panel ${k + 1} row ${i + 1} label`, text: r.label }); }
    ...
    const series: Series[] =
      ...
      : spec.type === 'budget' ? [...spec.segments.map((g) => one([g.value])), one([spec.total.value]), ...(spec.marker ? [one([spec.marker.value])] : [])]
      : spec.type === 'compare' ? spec.panels.flatMap((p) => p.rows.map((r) => one([r.value])))
